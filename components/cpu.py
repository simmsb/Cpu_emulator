...
class Registers:
    def __init__(self):
        ...
        self.register_memory = [0 for i in range(len(self.register_map))]
    ...
    def preprocess(self, commands):
        ...
        for i in temporary_commands:
            ...
            if self.instruction_set.encode_name(op):  # is a command
                ...
                return_commands.append(int(op)) # if it is a number, keep it anyway
    ...
    def compile(self):
        ...
        for c, i in enumerate(self.program):
            try:
                ...
                raise Exception("Invalid command passed: ({}) on line {}".format(i, c))
